...
#### 例题
##### 例1
若已知 $x(t)$ 的傅里叶变换为 $X(f)$，求 $x(t_0 - at)$ 的傅里叶变换 $(a > 0)$。

明显这是一个时间尺度改变和时移的结合。哪一个先来都可以，这里以先时间尺度改变后时移为例：
设$g(t) = x(-at)$，则$G(f) = \dfrac{1}{a}X\left(-\dfrac{f}{a}\right)$
显然有$g(t-\dfrac{t_0}{a}) = x(t_0 - at)$，则
$$F[x(t_0 - at)] = e^{-j2\pi f \frac{t_0}{a}} G(f) = \dfrac{1}{a} e^{-j2\pi f \frac{t_0}{a}} X\left(-\dfrac{f}{a}\right)$$
##### 例2
若已知 $x(t)$ 的 Fourier 变换为 $X(f)$，求 $t^2 x(t)$ 的傅里叶变换。
## 单位脉冲函数$\delta(t)$
